...
export default class App extends Component {
  ...
  toggleTodoItemCallback = (itemChecked) => this.setState({

    todoItems: this.state.todoItems.map( item2 => item2.action === itemChecked.action ? {...item2, done: !item2.done}:item2)
  });

  todoRowITems = () => this.state.todoItems.map( item => 
    <TodoRows key={item.action} item= {item} callback={this.toggleTodoItemCallback} /> )
